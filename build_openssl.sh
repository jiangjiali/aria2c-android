...
./Configure no-shared android-arm -D__ANDROID_API__=26 --prefix="$INSTALL_DIR/armeabi-v7a"
make $VERBOSE_FLAGS clean
make -j4 $VERBOSE_FLAGS
make install_sw

echo -e "\n++ Build openssl arm64-v8a ++"
./Configure no-shared android-arm64 -D__ANDROID_API__=26 --prefix="$INSTALL_DIR/arm64-v8a"
make $VERBOSE_FLAGS clean
make -j4 $VERBOSE_FLAGS
make install_sw
